    ...
    function(source, cb, society, amount, invoiceId, sendercitizenid)
        ...
        if Config.BillingCommissions[society] then
            ...
            local mailData = {
                ...
                subject = 'Erhaltene Provision',
                message = string.format('Sie haben einen Provisionsscheck erhalten von  $%s Wenn %s %s eine Rechnung bezahlt $%s.',
...
QBCore.Commands.Add('bill', 'Bill A Player', {{
    ...
    if biller.PlayerData.job.name == "police" or biller.PlayerData.job.name == 'ambulance' or biller.PlayerData.job.name == 'taxi' or biller.PlayerData.job.name == 'flywheels' or biller.PlayerData.job.name == 'mechanic' then
        if billed ~= nil then
...
AddEventHandler('qb-phone:server:AnswerCall', function(CallData, TargetData)
    local src = source
    local Play = QBCore.Functions.GetPlayer(src)
...
QBCore.Functions.CreateCallback('qb-phone:server:GetCurrentMechanic', function(source, cb)
    ...
    for k, v in pairs(QBCore.Functions.GetPlayers()) do
        local Player = QBCore.Functions.GetPlayer(v)
        if Player ~= nil then
            if Player.PlayerData.job.name == "flywheels" then
